...
function Aggregator() {
  ...
  useEffect(() => {
    ...
    swapAggregator.onIframeLoad(() => {
      setLoad(true);
      const url =
        "https://app.1inch.io/#/1/embedded-swap/1INCH/DAI?sourceTokenAmount=15&theme=dark";
    });
  }, []);
  return (
    <>
      {!loaded ? (
        <div className="a-container text-white text-center text-4xl w-full h-screen items-center h-100vh pt-32 min-h-max">
          Loading...
        </div>
      ) : (
        ""
